...
// kontrola ci sa p nachadza v hadovi
...
// kontrola ci p nekoliduje s inymi ovociami
...
static void move_one(game_t *G, player_t players[MAX_PLAYERS], player_t *pl) {
    ...
    // naraz do okraja
    if (nh.x >= G->map.w || nh.y >= G->map.h) { 
      pl->alive = 0;
      return; 
    }

    // zisti ci je na danej pozici ovocie
    int ate = -1;
    for (uint8_t i = 0; i < G->fruit_count; i++) {
        if (pos_eq(G->fruits[i], nh)) {
          ate = (int)i; 
          break; 
        }
    }
    ...
    // kolizia s hracom
    if (collides(G, players, pl, nh, ate >= 0)) {
      pl->alive = 0; 
      return; 
    }

    // pridanie skore a respawn ovocia
    if (ate >= 0 && pl->snake_len < MAX_SNAKE) {
        // posun doprava a rast o 1
        for (int k = (int)pl->snake_len; k > 0; k--){
          pl->snake[k] = pl->snake[k-1];
        }
        ...
        // posun bez rastu
        for (int k = (int)pl->snake_len - 1; k > 0; k--){
          pl->snake[k] = pl->snake[k-1];
        }
        pl->snake[0] = nh;
    }
...
void game_move_all(game_t *G, player_t players[MAX_PLAYERS]) {
    ...
    for (int i = 0; i < MAX_PLAYERS; i++){
      move_one(G, players, &players[i]);
    }
}

// zoberie game_t a player_t a vrati state_t
void game_make_state(const game_t *G, const player_t players[MAX_PLAYERS], state_t *out) {
    ...
    // skopirovanie suradnic koncov mapy do arrayu
    ...
    // spocitanie player count a active count
    // aby sme vedeli kedy je koniec
    uint8_t pc = 0, ac = 0;
    for (int i = 0; i < MAX_PLAYERS; i++) {
        if (players[i].active){
          pc++;
    ...
    for (uint8_t i = 0; i < G->fruit_count; i++){
      out->fruits[i] = G->fruits[i];
    }
      
    // info o hracovi a ich hadovi
    for (int i = 0; i < MAX_PLAYERS; i++) {
